  ...
  return {
    ...
    rewardType:
      ...
      item.rewardType === "other"
        ...
        : "xp",
    rewardAmount:
      typeof item.rewardAmount === "string" ? item.rewardAmount : undefined,
    rewardCurrency:
      typeof item.rewardCurrency === "string" ? item.rewardCurrency : undefined,
    rewardLabel:
      typeof item.rewardLabel === "string" ? item.rewardLabel : undefined,
    participantsJoined: Number(item.participantsJoined ?? 0),
    selectedParticipants: Array.isArray(item.selectedParticipants)
      ? (item.selectedParticipants as SponsorParticipant[])
      : [],
    startAt: typeof item.startAt === "number" ? item.startAt : undefined,
    accessOpensAt:
      typeof item.accessOpensAt === "number" ? item.accessOpensAt : undefined,
    accessToken:
      typeof item.accessToken === "string" ? item.accessToken : undefined,
    expiresAt: typeof item.expiresAt === "number" ? item.expiresAt : undefined,
    communityLinks: Array.isArray(item.communityLinks)
...
export function loadAllSponsorCampaigns(): SponsorCampaign[] {
  ...
  try {
    ...
    return saved
      ? (JSON.parse(saved) as Record<string, unknown>[]).map(migrate)
      : [];
  } catch {
    return [];
  }
  ...
  const remaining = loadAllSponsorCampaigns().filter(
    (campaign) =>
      !(
        ...
        normalizeAddress(campaign.ownerWallet) === normalizeAddress(walletAddress)
      ),
  );
